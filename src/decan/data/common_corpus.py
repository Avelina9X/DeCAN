...
# class LegacyCommonCorpusClientDataset( IterableDataset ):
#     def __init__(
#         self,
...
# class LegacyCommonCorpusDataset( IterableDataset ):
#     def __init__(
#         self,
#         tokenizer: PreTrainedTokenizerBase,
#         seq_length: int,
...
#         """
#         return LegacyCommonCorpusClientDataset(
...
class CommonCorpusClientDataset( IterableDataset ):
    """
    CommonCorpus Client IterableDataset which handles a single worker on a single device.
    
    Should be instantiated by the `CommonCorpusDataset` class to correctly set up arguments.
    """
    ...
    def line_iterator(
        ...
        # Iterate from current shard until end of the dataset
        for current_shard in range( starting_shard, 10_000, num_procs * world_size ):
            # Update current shard number to resume later (only worker zero updates this value)
            ...
            for i, line in enumerate( dataset ):
                ...
                # Yield only if (iterator rank) % (number of workers in world) equals worker id
                if i % ( num_procs * world_size ) == global_worker_id:
                    text = line[ 'text' ]
...
class CommonCorpusDataset( IterableDataset ):
    """
    CommonCorpus IterableDataset which spawns multiple workers on a single device.

    This dataset dynamically downloads `.parquet` from HuggingFace and will automatically delete each shard when fully
    consumed. This class manually handles splitting the batch size across multiple devices and automatically spawns
    `CommonCorpusClientDataset` instances based on the desired number of worker processes.
    """
